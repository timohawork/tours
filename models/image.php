...
class Image
{
	...
	public function render($options = array())
	{
		...
		return !empty($options['withViewLink']) ? '<a href="/'.str_replace($this->title, $this->bigTitle, $this->url).'" rel="prettyPhoto[pp_gal]" title="'.(!empty($options['title']) ? $options['title'] : '').'">'.$html.'</a>' : $html;
	}
	...
	public static function renderBlock($router, $url, $title, $withTitle = true, $isPreview = false)
	{
		...
		$href = empty($router) ? '' : (Router::TYPE_GALLERY !== $router->type ? '/album'.$router->url.'/'.$title : '');
